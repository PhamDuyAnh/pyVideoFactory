...
def test_smoke_render(tmp_path: Path):
    ...
    master = probe_media(tmp_path / "output/master.mp4")
    preview = probe_media(tmp_path / "output/preview.mp4")
    for info, resolution in ((master, (160, 284)), (preview, (540, 960))):
        assert info.has_video and info.has_audio
        assert (info.width, info.height) == resolution
        assert info.fps == pytest.approx(24, abs=0.01)
